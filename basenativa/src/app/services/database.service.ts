...
export class DatabaseService {
  ...
  constructor(private plt: Platform, private sqlitePorter: SQLitePorter, private sqlite: SQLite,
    ...
    addAutor(nombre, genero, img){
      ...
      return this.database.executeSql('INSERT INTO autor (nombre, genero, img) VALUES (?, ?, ?)',data).then(data =>{
        this.loadAutores();
      });
    }

    getAutor(id): Promise<AutorInt>{
      return this.database.executeSql('SELECT * FROM autor WHERE id = ?', [id]).then(data => {
        return {
    ...
    deleteAutor(id){
      return this.database.executeSql('DELETE FROM autor WHERE id = ?',[id]).then(_ => {
        this.loadAutores();
        this.loadLibros();
      });
    }

    updateAutor(id:number,nombre:string,genero:string,img:string){
      let data = [nombre,genero,img,id];
      console.log('database',data);
      return this.database.executeSql('UPDATE autor SET nombre = ?, genero = ?, img = ? WHERE id = ?', [nombre,genero,img,id]).then(_ =>{
        this.loadAutores();
        console.log('database2',nombre);
      })
    }
    ...
    deleteLibro(id){
      return this.database.executeSql('DELETE FROM obra WHERE id = ?',[id]).then(_ => {
        this.loadAutores();
        this.loadLibros();
      });
    }
